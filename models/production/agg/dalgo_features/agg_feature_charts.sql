WITH date_spine_raw AS (
    {{ dbt_utils.date_spine(
        datepart="day",
        start_date="DATE_TRUNC('month', (SELECT MIN(created_at) FROM " ~ source('django', 'ddpui_chart') ~ "))::date",
...
LEFT JOIN {{ source('django', 'ddpui_chart') }} c
...
GROUP BY
    ...
    c.chart_type
ORDER BY ds.date_day DESC, ds.org_slug, c.chart_type
